...
P.lens_thickness = 3 * P.lambda; % m
P.skin_wavespeed = 1600; % m/s
P.brain_wavespeed = 1570; % m/s

% bone properties
P.bone_wavespeed = 3000; % m/s
P.bone_thickness = 4e-3; % m
P.bone_curvature = 1; % 1/m

% distance to bone and pixel
P.distance_trans_bone = 3e-3; % mm 
...
% fig saving
output_folder_base = fullfile('figs', 'human_temporal_bone');
...
[metrics, data, figs2] = rt_test_improvement(P, BFC,out, true);
...
for k = 1:numel(P_all)
    ...
    % saving of figures
    output_folder = fullfile(output_folder_base, Ptest.name);
    [~, ~] = mkdir(output_folder);
